"""
reference: https://blog.csdn.net/Brikie/article/details/115602714
"""
...
mu = torch.Tensor([0.485, 0.456, 0.406]).unsqueeze(-1).unsqueeze(-1).cuda().unsqueeze(0)
std = torch.Tensor([0.229, 0.224, 0.225]).unsqueeze(-1).unsqueeze(-1).cuda().unsqueeze(0)
# pdb.set_trace()
...
for itera in range(20001):
    ...
    if itera%1000==0:    
        ...
        cv2.imwrite('demo/transfer%d.jpg'%itera,save_img)
